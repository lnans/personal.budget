...
- Typical request flow: Minimal API endpoint -> MediatR request -> handler in `Application.Features.*` -> domain methods/entities -> `IAppDbContext` persistence -> `ErrorOr<T>` mapped to HTTP Problem Details.
...
- Commands/queries use MediatR; write operations generally return `ErrorOr<Response>`.
...
- Validation pipeline is centralized in `ValidationBehavior<TRequest,TResponse>`; validators are auto-registered from the `Application` assembly.
